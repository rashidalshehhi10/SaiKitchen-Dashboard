        ...
        datatable = $('#kt_datatable').KTDatatable({
            ...
            columns: [{
                    ...
                    template: function(data) {;
                        return '<span class="font-weight-bolder">' + data.workScopeId + '</span>';
...

                {
                    ...
                    template: function(data) {
                        ...
                        if (data.code !== null) {
                            output += '<div class="font-weight-bolder text-primary mb-0">' + data.workScopeName + '</div>';
                            // output += '<div class="text-muted">' + status[index].title + '</div>';
                        }
                        return output;
                    },
                }, {
                    field: 'workScopeDescription',
                    title: 'Description',
                    autoHide: true,
                    // type: 'date',
                    // format: 'MM/DD/YYYY',
                    template: function(data) {
                        ...
                        // };
                        // var index = KTUtil.getRandomInt(1, 4);
                        if (data.workScopeDescription != null) {
                            output += '<div class="font-weight-bold text-muted">' + data.workScopeDescription + '</div>';
                            // output += '<div class="text-muted">' + status[index].title + '</div>';
                        }
                        return output;
                    },
                    ...
                    template: function(data) {
                        ...
                        return '<span class="label label-lg font-weight-bold ' + status[data.questionaireType].class + ' label-inline">' + status[data.questionaireType].title + '</span>';
                    ...
                    autoHide: false,
                    template: function(data) {
                        if (workscopePermission >= 3) {
                            var action = '\
<button type="button"  onclick="getWorkScopeById(\'' + baseURL + '\',\'' + data.workScopeId + '\');"  class="btn btn-sm btn-default btn-text-primary btn-hover-primary btn-icon mr-2" title="Edit details">\
    ...
    var _handleFormAddCustomer = function() {
        ...
        FormValidation
            ...
            .on('core.form.valid', function() {
                ...
                workscope.workScopeId = document.getElementById("workScopeId").innerHTML;
                workscope.workScopeName = document.getElementById('workScopeName').value;
                workscope.workScopeDescription = document.getElementById('workScopeDescription').value;
                workscope.questionaireType = $('#kt_questionaire_type').val();

                // console.log(branchRole);
                const data = JSON.stringify(workscope);
                console.log(data);
                $.ajax({
                    type: "Post",
                    url: baseURL + '/Inquiry/AddWorkscope',
                    ...
                    success: function(response) {
                        ...
                        if (response.isError == false) {
                            ...
                            window.location.replace("workscope.html");
...
jQuery(document).ready(function() {
    ...
    if (login !== null) {
        ...
        for (var i = 0; i < permissions.length; i++) {
            if (permissions[i].permissionId == 10) {
                workscopePermission = permissions[i].permissionLevelId;
                console.log(workscopePermission);
            }
        }
