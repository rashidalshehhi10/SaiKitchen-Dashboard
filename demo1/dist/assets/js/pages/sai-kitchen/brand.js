        ...
        datatable = $('#kt_datatable').KTDatatable({
            ...
            columns: [{
                    ...
                    template: function(data) {;
                        return '<span class="font-weight-bolder">' + data.brandId + '</span>';
                ...
                },
             {
                ...
                },
                 {
                    ...
                    template: function(data) {
                        ...
                        if (data.brandDescription != null) {
                            output += '<div class="font-weight-bold text-muted">' + data.brandDescription + '</div>';
                        }
                        return output;
                    },
                ...
                },
                {
                    ...
                    autoHide: false,
                    template: function(data) {
                        if (brandPermission >= 3) {
                            var action = '\
<button type="button"  onclick="getbrandById(\'' + baseURL + '\',\'' + data.brandId + '\');"  class="btn btn-sm btn-default btn-text-primary btn-hover-primary btn-icon mr-2" title="Edit details">\
		...
		';
        action += '\
			<button type="button"  onclick="deleteBrandById(\'' + baseURL + '\',\'' + data.brandId + '\');"  class="btn btn-sm btn-default btn-text-primary btn-hover-primary btn-icon" title="Delete Brand">\
			<span class="svg-icon svg-icon-md">\
				<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
					<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
						<rect x="0" y="0" width="24" height="24"/>\
    ...
    var _handleFormAddBrand = function() {
        var form = KTUtil.getById('kt_add_brand');
        var formSubmitUrl = KTUtil.attr(form, 'action');
        var formSubmitButton = KTUtil.getById('kt_add_brand_button');
            ...
            .on('core.form.valid', function() {
              ...
              //  console.log($('input[name="measurementPromo"]:checked').val());
               // var measurementPromo=$('input[name="measurementPromo"]:checked').val()=="on"?true:false;
                ...
                var brand = {
                    brandId:  document.getElementById("brandId").innerHTML,
    ...
    if (login !== null) {
        ...
        for (var i = 0; i < permissions.length; i++) {
            if (permissions[i].permissionId == 12) {
                brandPermission = permissions[i].permissionLevelId;
                console.log(brandPermission);
        ...
        if (brandPermission >= 2) {
            document.getElementById('btnNewCustomer').innerHTML += `	<!--begin::Button-->
			<button type="button" onclick="customFormReset();" data-target="#addBrand" data-toggle="modal"
